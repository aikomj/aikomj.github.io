...
registry {
  ...
  eureka {
    serviceUrl = "http://127.0.0.1:10001/eureka"
    application = "icodingedu-seata-tc"
    weight = "1"
  }
  redis {
    serverAddr = "localhost:6379"
...
service {
  ...
  vgroup_mapping.icodingedu_tx_group = "icodingedu-seata-tc"
  #only support when registry.type=file, please don't set multiple addresses
  default.grouplist = "127.0.0.1:8091"
  #degrade, current not support
  enableDegrade = false
...
修改完后将registry.conf和file.conf重新导入到resources

将服务的注册中心配置和POM依赖修改为eureka

```shell
2020-07-09 22:01:34.710 INFO [ServerHandlerThread_1_500]io.seata.server.coordinator.DefaultCore.doGlobalRollback:465 -Successfully rollback global, xid = 192.168.1.5:8091:2048030665
```
