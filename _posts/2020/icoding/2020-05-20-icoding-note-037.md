...
<mark>过滤器中都是0和1，所以说，你无法查询这个数据是什么，只能查询它是够存在，然后还发现，这个数据不能删除</mark>
...
最终调用redis pexpire命令给key(锁名称)续命30000毫秒，不过前面`trylock`的时候参数leaseTime=-1才会启动watchDog机制给锁续命，其实就是没隔10秒重新设置redis 的key过期时间为30000毫秒后。
