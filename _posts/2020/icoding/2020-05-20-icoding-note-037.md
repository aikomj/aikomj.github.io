...
tags: [icoding-edu]
keywords: redis
excerpt: Jedis、SpringBoot整合Redis，缓存击穿、穿透、雪崩及解决方案，布隆过滤器，Redisson分布式锁，Redis6的新特性
lock: noneed
---
...
- expire 过期
  ...
  ![](/assets/images/2020/jedis/expire.gif)

  ![](/assets/images/2020/jedis/setex.gif)

- Incr 增加

  ![](/assets/images/2020/jedis/incr.gif)

- list

  ![](/assets/images/2020/jedis/list.gif)

- zset

  ![](/assets/images/2020/jedis/zset.gif)

- bitmap

  ![](/assets/images/2020/jedis/bitmap.gif)

- geo

  ![](/assets/images/2020/jedis/geo.gif)

- hyperloglog

  ![](/assets/images/2020/jedis/hperloglog.gif)
...
![](/assets/images/2020/jedis/global-search-redisautoconfiguration.gif)
...
![](/assets/images/2020/jedis/redis-ops.gif)
...
我的问题是应该将什么样的数据放到布隆过滤器中，也就是第一步数据预热的问题。
...
==过滤器中都是0和1，所以说，你无法查询这个数据是什么，只能查询它是够存在，然后还发现，这个数据不能删除==
...
相比于传统的 List、Set、Map 等数据结构，==它更高效、占用空间更少==，但是==缺点是其返回的结果是概率性的，而不是确切的==。
...
2、防止死锁（在分布式高并发的情况下，有线程获得锁的同时，还没有来的去释放锁，现在系统故障了，其他线程都无法获取，死锁），因此所有的锁都需要设置==有效时间==，解决死锁问题。
...
参考：[https://www.cnblogs.com/qdhxhz/p/11046905.html](https://www.cnblogs.com/qdhxhz/p/11046905.html)
