...
服务限流，统一限流异常处理类SentinelBlockHandler

```java
package com.icodingedu.supermall.service;
...
public class SentinelBlockHandler {
    //注意这里的本地存根方法必须要加上static
    public static String hotkeyFallback(String p1,String p2,BlockException exception){
        return "{\"status\":\"403\",\"msg\":\"sorry 梁军杰 limited\"}";
    }
}
```

web层接口指定限流异常处理类和具体的方法

```java
@GetMapping("testhot") //地址作为资源名需要加斜杠/testhot
@SentinelResource(value = "hotkey",
                  blockHandlerClass = SentinelBlockHandler.class,
                  blockHandler = "hotkeyFallback")
public String testHotKey(@RequestParam(value="p1",required=false) String p1,
                         @RequestParam(value="p2",required=false) String p2){
  return "返回的testhotkey:参数1:"+p1+"  参数2:"+p2;
}
...
![](/assets/images/2020/springcloud/sentinel-fallback.jpg)

结合流控、降级规则一起使用，当满足规则，就会执行fallback方法

![](/assets/images/2020/springcloud/sentinel-fallback-1.jpg)

当1秒访问超过1次，就发触发

![](/assets/images/2020/springcloud/sentinel-fallback-2.jpg)



## 9、结合FeignClient使用

这篇博客 ：http://139.199.13.139/blog/springcloud/2019/12/18/springcloud-sentinel.html