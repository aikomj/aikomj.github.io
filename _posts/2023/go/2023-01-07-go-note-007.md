...
![](/assets/images/2022/go/zhizhen.png)
...
func main() {
	...
	fmt.Println("指针变量p存储的内存地址：", p)
	fmt.Println("指针变量p的内存地址：", &p)
	fmt.Println("*p变量的值：", *p)
...
go语言中，<mark>结构体是值类型的</mark>，java中类的使用都是引用类型的
...
- 如果结构体名称首字母大写，则结构体可被导出，但只会到处大写首字母的成员字段，那些小写首字母的成员字段不会被导出
- 如果存在嵌套结构体，即使嵌套的结构体字段首字母小写，外部也能访问到其中首字母大写的成员字段

正常开发，会把结构体放到一个独立的package中

![](/assets/images/2022/go/user-struct.png)

```go
package pojo

// 大写首字母就是外部可以访问的 public
type User struct {
	Name  string
...
![](/assets/images/2022/go/user-struct-2.png)