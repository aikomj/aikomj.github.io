...
mycat是服务端代理模式的数据切分方案，shardingsphere则是客户端代理模式的数据切分方案，mybatis-plus最近发布的mybatis-mate，也是客户端代理模式的数据切分方案，跟shardingsphere真的好像，支持分库分表，数据审计、数据敏感词过滤（AC算法），字段加密，字典回写（数据绑定），数据权限，表结构自动生成 SQL 维护等，旨在更敏捷优雅处理数据。主要功能
...
更多了解还是需要看官方文档 [https://baomidou.com/guide/mybatis-mate.html](https://baomidou.com/guide/mybatis-mate.html)

![](/assets/images/2021/springcloud/mybatis-mate.png)

官方例子地址：[https://gitee.com/baomidou/mybatis-mate-examples](https://gitee.com/baomidou/mybatis-mate-examples)

springboot项目整合使用，pom.xml导入依赖
...
@Sharding 注解使数据源不限制随意使用切换，你可以在 mapper 层添加注解
