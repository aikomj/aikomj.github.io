...
excerpt: 高并发意味着大流量衍生出的一些技术问题，设计目标高性能、高可用、高扩展，实践方案，纵向扩展，横向扩展，针对三高目标的具体实践方案
lock: noneed
---
...
高并发意味着大流量，随着流量变大，会遇到各种各样的技术问题，比如接口响应超时、CPU load升高、GC频繁、死锁、大数据量存储等等，这些问题能推动我们在技术深度上不断精进。

我们需要运用技术手段抵抗流量的冲击，这些手段好比操作流量，能让流量更平稳地被系统所处理，带给用户更好的体验。
...
![](\assets\images\2021\architect\high-concurrent.png)
...
- 平均响应时间
  ...
  最常用，但是缺陷很明显，对于慢请求不敏感。比如1万次请求，其中9900次是1ms，100次是100ms，则平均响应时间为1.99ms，虽然平均耗时仅增加了0.99ms，但是1%请求的响应时间已经增加了100倍。

- TP90、TP99等分位值：将响应时间按照从小到大排序，TP90表示排在第90分位的响应时间， 分位值越大，对慢请求越敏感。

  ![](\assets\images\2021\architect\high-concurrent-1.jpg)
...
![](\assets\images\2021\architect\high-concurrent-2.png)
...
小结：上述方案无外乎从计算和 IO 两个维度考虑所有可能的优化点，需要有配套的监控系统实时了解当前的性能表现，并支撑你进行性能瓶颈分析，然后再遵循二八原则，抓主要矛盾进行优化。
...
小结：高可用的方案主要从冗余、取舍、系统运维3个方向考虑，同时需要有配套的值班机制和故障处理流程，当出现线上问题时，可及时跟进处理。
