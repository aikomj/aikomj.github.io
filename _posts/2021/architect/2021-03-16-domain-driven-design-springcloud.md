...
- 边界内的内容具有一致性：<mark>在一个事务中只修改一个聚合实例</mark>。如果你发现边界内很难接受强一致，不管是出于性能或产品需求的考虑，应该考虑剥离出独立的聚合，采用最终一致的方式。
- 设计小聚合：<mark>大部分的聚合都可以只包含根实体</mark>，而无需包含其他实体。即使一定要包含，可以考虑将其创建为<mark>值对象</mark>。
...
- 应用层
  ...
  尽量简单，本身不包含业务规则，**主要面向用例操作**，它协调和指挥领域层的领域对象来完成业务逻辑。提供与业务无关的服务，如安全认证、权限校验、分布式和持久化事务控制或向外部应用发送基于事件的消息等（领域事件）。
